...
> 현재 원인모를 이유로 `remark-math`, `rehype-katex` 모듈 작동이 실패하기 때문에 수식을 쓸 때 이를 위해 만들어 놓은 Katex 컴포넌트를 사용해야 합니다.
>
> `.mdx` 문서 내에서 다음과 같이 사용합니다.
>
>
> `import {KatexBlock, KatexInline} from 'components/Katex'`
> `<KatexInline formula="T_n = a + (n-1)d" /> // 인라인 모드`
> `<KatexBlock formula="E = m c^{2}" />       // 블럭 모드`

import {KatexBlock, KatexInline} from 'components/Katex'

#### 인라인 모드

문장 내의 수식: <KatexInline formula="T_n = a + (n-1)d" />

#### 블럭 모드

<KatexBlock formula="E = m c^{2}" />
<KatexBlock formula="Mean = \frac{10 + 14+ 7 +23+ 23+ 15 + 7 + 23 + 32}{9} = \frac{154}{9} \approx 17.11" />

### 2-3. GFM(Github Flavored Markdown)

GFM문법을 지원하여 Table 등의 다양한 서식을 표현할 수 있습니다.
